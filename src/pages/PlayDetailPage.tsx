...
export const PlayDetailPage: React.FC<PlayDetailPageProps> = ({ onOpenLogModal }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-8">
      ...
      <nav className="flex items-center gap-2 text-xs font-mono text-text-secondary overflow-x-auto no-scrollbar whitespace-nowrap py-0.5">
      ...
      <div className="bg-canvas border border-border-subtle rounded-sm p-4 sm:p-8">
        <div className="flex flex-col md:flex-row gap-5 sm:gap-8 items-center md:items-start">
          ...
          <div className="flex-1 w-full space-y-3.5">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0 self-start mt-1 sm:mt-0">
                  <span className="text-[10px] font-mono uppercase tracking-wider text-theatre-curtain font-semibold bg-theatre-curtain/10 px-2 py-0.5 rounded-sm border border-theatre-curtain/20">
                    {play.genre}
                  </span>
                  <span className="text-[10px] font-mono font-bold text-text-secondary bg-layer-01 px-2 py-0.5 rounded-sm border border-border-subtle">
                    {play.year}
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between gap-2 py-2 border-y border-border-subtle">
              ...
              {play.rating >= 4.5 && (
                <span className="inline-flex items-center gap-1 font-mono text-[10px] font-bold px-2 py-0.5 rounded-sm bg-theatre-gold/15 text-amber-700 dark:text-theatre-gold border border-theatre-gold/30 shrink-0 whitespace-nowrap">
                  <Award className="w-3 h-3 text-theatre-gold" />
            ...
            {/* Action Center - Unified Clean Modern Layout */}
            <div className="pt-2 space-y-2">
              {/* Primary CTA: Not Ekle */}
              <button
                type="button"
                onClick={() => onOpenLogModal?.(play)}
                className="w-full inline-flex items-center justify-center gap-2 bg-theatre-curtain hover:bg-theatre-curtain-hover text-white py-2.5 px-4 text-xs font-semibold rounded-sm shadow-xs transition-colors cursor-pointer"
              >
                <StickyNote className="w-4 h-4 stroke-[2]" />
                <span>Bu Oyuna Not Ekle</span>
              </button>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-1.5 py-2 px-2 text-xs font-medium rounded-sm border transition-all cursor-pointer ${
                    isSeen
                      ? 'bg-success-mint/10 border-success-mint text-success-mint font-semibold shadow-xs'
                      : 'bg-layer-01 hover:bg-layer-02 text-text-secondary hover:text-text-primary border-border-subtle'
                  }`}
                  title={isSeen ? 'İzlendi olarak kayıtlı' : 'İzledim olarak işaretle (+10 XP)'}
                >
                  <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                  <span>{isSeen ? 'İzledim' : 'İzledim'}</span>
                </button>
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-1.5 py-2 px-2 text-xs font-medium rounded-sm border transition-all cursor-pointer ${
                    isWatchlisted
                      ? 'bg-theatre-curtain/10 border-theatre-curtain text-theatre-curtain font-semibold shadow-xs'
                      : 'bg-layer-01 hover:bg-layer-02 text-text-secondary hover:text-text-primary border-border-subtle'
                  }`}
                  title={isWatchlisted ? 'İzleme listende' : 'İzlemek istediklerime ekle'}
                >
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1.5 py-2 px-2 text-xs font-medium bg-layer-01 hover:bg-layer-02 text-text-secondary hover:text-text-primary border border-border-subtle rounded-sm transition-colors cursor-pointer"
                  title="Afişi veya notları görsel kart olarak paylaş"
                >
