...
export const MobileTabBar: React.FC<MobileTabBarProps> = ({ onOpenLogModal }) => {
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-tn-surface/95 backdrop-blur-md border-t border-tn-border/60 pb-[env(safe-area-inset-bottom)] px-3 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]"
    >
      <div className="h-[64px] flex items-center justify-around font-serif text-tn-text">
        {/* Katalog */}
        <Link
          to="/"
        ...
        {/* Center: Not Ekle (+) Button */}
        <button
          type="button"
          onClick={onOpenLogModal}
          aria-label="Not Ekle"
          className="-mt-5 w-14 h-14 rounded-full bg-tn-red text-white flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-transform cursor-pointer border-none"
        >
